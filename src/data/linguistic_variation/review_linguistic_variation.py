"""Quality review pipeline for linguistic variation sentence pairs.

Uses the Cohere ``chat`` API to evaluate each generated pair against
type-specific criteria (lexical, syntactic, semantic) and produces
per-pair verdicts with pass/fail reasons.  Also detects duplicates
across the full dataset by normalised ``sentence_1``.

Requires ``COHERE_API_KEY`` in the environment.

Usage::

    python -m src.data.linguistic_variation.review_linguistic_variation
"""
...
#: Validation criteria for **lexical** pairs.
...
#: Validation criteria for **syntactic** pairs.
...
#: Validation criteria for **semantic** pairs.
...
#: Maps variation type name to its validation criteria string.
CRITERIA_BY_TYPE = {
...
class PairVerdict(BaseModel):
    """Review verdict for a single sentence pair.

    Attributes
    ----------
    pair_id : str
        Identifier of the reviewed pair.
    valid : bool
        *True* if the pair satisfies all criteria for its type.
    reason : str
        One-sentence justification (explains which criterion failed
        when *valid* is *False*).
    """
...
class BatchVerdict(BaseModel):
    """Container of per-pair verdicts returned by the Cohere API.

    Attributes
    ----------
    verdicts : list[PairVerdict]
        One verdict per pair in the reviewed batch.
    """
...
class LinguisticVariationReviewer:
    """Review generated sentence pairs for quality using the Cohere API.
    ...
    def review_batch(self, variation_type: str, pairs: list[dict]) -> list[PairVerdict]:
        """Review a single batch of pairs via the Cohere API.
        ...
        pairs : list[dict]
            Dicts with ``"pair_id"``, ``"sentence_1"``, ``"sentence_2"``.

        Returns
        -------
        list[PairVerdict]
            One verdict per input pair.
        """
...
- reason: one sentence explaining the verdict (be specific about which criterion fails if invalid)"""
    ...
    def review(
        ...
        """Review all pairs in a dataset file and write verdicts.

        Groups pairs by type, sends them through :meth:`review_batch`
        in chunks of *batch_size*, detects cross-type duplicates by
        normalised ``sentence_1``, and aggregates a per-type summary.

        Parameters
        ----------
        input_path : str or Path
            JSON file produced by :class:`LinguisticVariationPipeline`.
        output_path : str, Path, or None
            If given, write the full results (summary + verdicts) to
            this JSON file.
        batch_size : int
            Pairs per API call during review.

        Returns
        -------
        dict
            ``{"summary": {type: stats}, "verdicts": [...]}``.
        """
